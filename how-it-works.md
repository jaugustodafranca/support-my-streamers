...
1. **Play start** — shuffle live selected channels once → `queueOrder`; open first `SLOTS`.
2. **Each rotation tick** — refresh who is live; move finished channels to the back; append newly live to the end; pick next `SLOTS` from the front (FIFO).
3. **Between ticks** — `reconcileTabs` still handles offline/raid immediately.
...
```
t=0   → shuffle → opens A and B
t=5   → queue [C,D,A,B] → opens C and D
t=10  → queue [A,B,C,D] → opens A and B
```
...
- OAuth implicit grant — pure helpers in `src/auth.js`; `chrome.identity` flow in `background.js`.
